...
describe('API Service Layer tests', () => {
  ...
  describe('apiFetch and endpoints', () => {
    ...
    it('should handle logout correctly', async () => {
      ...
      await api.logout();
      expect(global.fetch).toHaveBeenCalledWith(
        expect.stringContaining('/api/v1/account/authentication/logout'),
    ...
    it('should handle non-JSON error responses gracefully', async () => {
      ...
      await expect(api.getEmployees()).rejects.toThrow('HTTP error! status: 500');
    });
    ...
    it('should handle 204 No Content responses', async () => {
      ...
      const response = await api.deleteOrganisation('123');
      expect(response).toBeNull();
    });

    it('should handle non-JSON successful responses gracefully', async () => {
      global.fetch.mockResolvedValueOnce({
        ok: true,
        status: 200,
        json: async () => { throw new Error('Not JSON'); },
    ...
    it('should handle network failures', async () => {
      ...
      await expect(api.getEmployees()).rejects.toThrow('Failed to fetch');
    });
  });
});
